...
public class ExcelDataUtil {
	...
	// private static int columnToLookTestCaseID =
	// Integer.parseInt(ConfigReader.getValue("columnToLookTestCaseID"));
	private static String testDatafilePath = "C:\\Users\\mabduholikov0418\\Desktop\\NewBDDTemplate\\backup\\FCBT_Selenium_TestAutomation_Framework\\src\\test\\resources\\testData\\TestData.xlsm";
	// private static String testDataSheetName = "TestData";
	 ...
	 * @return
	 */
	public static HashMap<String, String> ReadTestDataFromExcel(String sheetName) {
		...
		try {
			...
			while (rowIterator.hasNext()) {
				...
				System.out.println(row.getCell(0).getStringCellValue() + "-" + cellValue);
				currentRowData.put(row.getCell(0).getStringCellValue(), cellValue);
	...
	/*
	 * public static String ReadTestDataFromExcel(String key) {
	 * 
	 ...
	 * XSSFWorkbook wb = new XSSFWorkbook(inputStream); XSSFSheet wsh1 =
	 * wb.getSheetAt(0); DataFormatter formatter = new DataFormatter(); int rowNum =
	 * wsh1.getPhysicalNumberOfRows(); System.out.println(rowNum);
	 * 
	 * for (int i = 0; i < rowNum; i++) { Cell keyColCell =
	 * wsh1.getRow(i).getCell(0); String cellValueStr =
	 * formatter.formatCellValue(keyColCell); if (cellValueStr.equals(key) == true)
	 * { Cell ValueColCell = wsh1.getRow(i).getCell(1); val =
	 * formatter.formatCellValue(ValueColCell);
	 * 
	 * }
	 * 
	 * 
	 ...
	 * 
	 * 
	 * } catch (Exception e) { LogUtil.errorLog(ExcelDataUtil.class,
	 * "caught exception", e); }
	 * 
	 * return val; }
	 */
	 ...
	 */
	/*
	 * public static CommonSettings getCommonSettings() { // 1. Read Excel File
	 ...
	 * FileInputStream(ConfigReader.getValue(automationcontrolexcelpath)); Workbook
	 ...
	 * 
	 * // 1. Set Application type Column[B17] Row =16, Column =1 String val =
	 * sheet.getRow(1).getCell(1).getStringCellValue();
	 * commonSettings.setAppType(val);
	 * 
	 * // 2. Set Application environment type Column[B18] Row =18, Column=1 val =
	 * sheet.getRow(2).getCell(1).getStringCellValue();
	 * commonSettings.setAppEnviornment(val);
	 * 
	 * // 3. Set Email output(Y/N) Column[B20] Row =19, Column =1 val =
	 * sheet.getRow(3).getCell(1).getStringCellValue();
	 * commonSettings.setEmailOutput(val);
	 * 
	 * // 4. Set Email Id Comma List Column[B25] Row =24, Column =1 val =
	 * sheet.getRow(4).getCell(1).getStringCellValue();
	 * commonSettings.setBuildNumber(val);
	 * 
	 * // 4. Set Email Id Comma List Column[B25] Row =24, Column =1 val =
	 * sheet.getRow(5).getCell(1).getStringCellValue(); commonSettings.setUrl(val);
	 * 
	 * // 4. Set Email Id Comma List Column[B25] Row =24, Column =1 val =
	 * sheet.getRow(14).getCell(1).getStringCellValue();
	 * commonSettings.setEmailIds(val);
	 * 
	 * // 5. Set Html report (Y/N) Column[B26] Row =25, Column =1 val =
	 * sheet.getRow(15).getCell(1).getStringCellValue();
	 * commonSettings.setHtmlReport(val);
	 * 
	 * // 6. Set XLS report (Y/N) Column[B27] Row =26, Column =1 val =
	 * sheet.getRow(16).getCell(1).getStringCellValue();
	 * commonSettings.setXlsReport(val);
	 * 
	 * // 7. Set Test Logs (Y/N) Column[B28] Row =27, Column =1 val =
	 * sheet.getRow(17).getCell(1).getStringCellValue();
	 ...
	 * 
	 * 
	 * } // End try catch (Exception e) { LogUtil.errorLog(ExcelDataUtil.class,
	 * EXCEPTIONCAUGHT, e); } return commonSettings;
	 * 
	 * }
	 */
	 ...
	 */
	/*
	 * public static String getFlagExcel(String sheetName, String searchValue) { int
	 * sheetSize = 0; int rowNum = 1; String strVal = ""; String strflag = "NA"; try
	 ...
	 * if (wb.getSheetIndex(wb.getSheet(sheetName)) == -1) {
	 * LogUtil.infoLog(ExcelDataUtil.class, INVALID_SHEET_MESSAGE + sheetName);
	 * throw new InvalidSheetException("No such sheet available in Excel file: " +
	 * sheetName); }
	 * 
	 * Sheet sheet = wb.getSheet(sheetName); sheetSize = sheet.getLastRowNum();
	 * 
	 * for (int i = rowNum; i <= sheetSize; i++) {
	 * 
	 * strVal = sheet.getRow(i).getCell(0).getStringCellValue();
	 * 
	 * if (searchValue.equalsIgnoreCase(strVal)) { strflag =
	 * wb.getSheet(sheetName).getRow(i).getCell(1).getStringCellValue(); break; } }
	 * 
	 * } catch (Exception e) { strflag = "NA"; LogUtil.errorLog(ExcelDataUtil.class,
	 * EXCEPTIONCAUGHT, e); }
	 * 
	 * return strflag; }
	 */
	 ...
	 */
	/*
	 * public static boolean isSuiteRunnable(String suiteName) { boolean isRunnable
	 * = false; String strVal; try { strVal = getFlagExcel("SuiteControlSheet",
	 * suiteName); if (strVal.equalsIgnoreCase(Y)) { isRunnable = true; } else {
	 * isRunnable = false; } } catch (Exception e) { isRunnable = false;
	 ...
	 * boolean isRunnable = false; String strVal = null; try { strVal =
	 * getFlagExcel(suiteName, scriptName); if (strVal.equalsIgnoreCase(Y)) {
	 * isRunnable = true; } else { isRunnable = false; } } catch (Exception e) {
	 * isRunnable = false; LogUtil.errorLog(ExcelDataUtil.class, EXCEPTIONCAUGHT,
	 * e); } return isRunnable; }
	 */
	 ...
	 * 
	 * if (suiteName.trim().isEmpty()) { LogUtil.infoLog(ExcelDataUtil.class,
	 * "Suite name not found!!!"); return false; }
	 * 
	 * if (testCaseID.trim().isEmpty()) { LogUtil.infoLog(ExcelDataUtil.class,
	 * "Test name is not specified!!!"); return false; }
	 * 
